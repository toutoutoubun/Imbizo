"""Adapter for MasakhaPOS / AfricaPOS CoNLL data."""
...
DISCLAIMER = "Dataset license is CC-BY-NC-4.0: non-commercial use only; check institutional and publication requirements before redistribution."
CAVEATS = (
    "This MasakhaPOS index was generated automatically from local CoNLL files. "
    "Entries are unverified; treat them as starting suggestions, not authoritative "
    "claims. POS tags may help 4-M review, but they do not decide morpheme type."
)
ISO_CODES = {"zul", "xho", "sot", "tsn", "nso"}


class MasakhaPOSAdapter(SourceAdapter):
    """Copy MasakhaPOS CoNLL files and write language indexes."""
    ...
    def convert(self, raw_path: Path, output_dirs: list[Path], source_metadata: Mapping[str, Any]) -> list[Path]:
        ...
        if not output_dirs:
            raise ValueError("MasakhaPOS conversion requires corpora/masakhapos output directory.")
        root = output_dirs[0]
        ...
        for iso, lang_files in sorted(by_lang.items()):
            index = write_index(
                ...
                languages=[iso],
                usable_by=["v1.0_four_m_auxiliary_pos", "evaluation"],
                source_metadata=source_metadata,
                raw_path=raw_path,
                adapter_path="tools/adapters/masakhapos.py",
        ...
        provenance = write_processing_provenance(str(source_metadata.get("id", "masakhapos")), raw_path, written, source_metadata)
